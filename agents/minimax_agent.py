...
class MinimaxAgent:
    ...
    def __init__(self, player_id: int, use_alpha_beta: bool = True,
    ...
    def select_action(self, game, training: bool = False) -> Union[Tuple[int, int], int]:
        ...
        # in case agent runs out of time/depth, play a fallback move
    ...
    def _max_value(self, game, depth: int, alpha: float, beta: float) -> float:
        ...
        for action in valid_actions:
            ...
            if value >= beta:
                return value  # return if beta cutoff
    ...
    def _min_value(self, game, depth: int, alpha: float, beta: float) -> float:
        ...
        for action in valid_actions:
            ...
            if value <= alpha:
                return value # return if alpha cutoff
    ...
    def _evaluate(self, game) -> float:
        ...
        if game.is_terminal():
            ...
            if winner == self.player_id:
                return 1.0  # the agent won
            elif winner == self.opponent_id:
                return -1.0  # the agent lost
            else:
                return 0.0  # the game is a draw

        # for depth-limited search, use a custom heuristic evaluation
        if hasattr(game, 'rows') and hasattr(game, 'cols'):
            # heuristic for Connect4
            return self._evaluate_connect4(game)
        else:
            # heuristic for TicTacToe
    ...
    def _evaluate_connect4(self, game) -> float:
        ...
        # check horizontal, vertical, and both diagonals for wins
        ...
        # for diagonal (in positive slope)
        for row in range(game.rows - 3):
            for col in range(game.cols - 3):
                window = [board[row+i, col+i] for i in range(4)]
                score += self._evaluate_window(window)

        # for diagonal (in negative slope)
    ...
    def _evaluate_tictactoe(self, game) -> float:
        """
        Custom heuristic evaluation for TicTacToe.
        Simpler than Connect4 since the state space is smaller.
        """
        board = game.board
        score = 0

        # check rows, columns, and diagonals for potential wins below
