...
class QLearningAgent:
    ...
    def state_to_tuple(self, game) -> tuple:
        ...
        if hasattr(game, 'rows') and hasattr(game, 'cols'):
            # use a more compact representation for Connect4
            # we can't use the raw board as a key because it's too large
            # instead, we can create a tuple of tuples where each inner tuple represents a column
            columns = []
            ...
            # use a flattened board for TicTacToe
            # convert board to tuple of tuples to hash it
    ...
    def update_q_value(self, state: tuple, action: Union[Tuple[int, int], int], 
        ...
        # else, get max Q-value for next state
        if done:
            max_next_q = 0
        else:
        ...
        # update the Q-learnign formula
        new_q_value = q_value + self.learning_rate * ( \
            reward + self.discount_factor * max_next_q - q_value)
    ...
    def select_action(self, game, training: bool = False) -> Union[Tuple[int, int], int]:
        ...
        # save current state for learning update
        if training:
            self.current_state = state

        # let agent explore with random action
        ...
        # let agent take best known action
